...
def inference(input_data, model, device, scale, img_range=255, save_dir=None):
    ...
    for idx, input_data_path in enumerate(input_data):
        ...
        input_nii = sitk.ReadImage(input_data_path)
        origin = input_nii.GetOrigin()  # 这三句是获取的原始图像文件的位置和方向吧。
        spacing = input_nii.GetSpacing()
        direction = input_nii.GetDirection()
        input_nii_data = sitk.GetArrayFromImage(input_nii)
        new_spacing = tuple([spacing[0] / scale, spacing[1] / scale, spacing[2]])
        rlt = []
        for jdx in tqdm.tqdm(range(input_nii_data.shape[0])):
            img = input_nii_data[jdx:jdx + 1, :, :].repeat(3, axis=0)
            _min, _max = img.min(), img.max()
            img = normalize(_min, _max, img).astype(np.float32) * 255.
            img_torch = torch.tensor(img).unsqueeze(0).to(device)
        ...
        flow_nii_data = np.stack(rlt, axis=0)
        new_nii_data = sitk.GetImageFromArray(flow_nii_data)
        new_nii_data.SetSpacing(new_spacing)
        new_nii_data.SetOrigin(origin)
        new_nii_data.SetDirection(direction)
        sitk.WriteImage(new_nii_data, output_nii_path)
