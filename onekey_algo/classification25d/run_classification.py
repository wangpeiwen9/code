...
from onekey_algo.datasets.ClassificationDataset import create_classification_dataset
...
def train_model(model, device, dataloaders, batch_size, criterion, optimizer, lr_scheduler, num_epochs=25,
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            running_size = 0
            # Iterate over data.
            for inputs, clinic, labels, fnames in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if is_inception and phase == 'train':
                        outputs, aux_outputs = model({'image': inputs, 'clinic': clinic})
                        loss1 = criterion(outputs, labels)
                        loss2 = criterion(aux_outputs, labels)
                        loss = loss1 + 0.4 * loss2
                    else:
                        outputs = model({'image': inputs, 'clinic': clinic})
...
def main(args):
    ...
    image_datasets = {'train': create_classification_dataset(
        ...
        check_sample_exists=get_param_in_cwd('check_sample_exists', True), is_25d=True,
        fname_len=args.fname_len,
    ...
    model = create_model(f'classification25d.{args.model_name}', num_classes=image_datasets['train'].num_classes,
                         pretrained=args.pretrained if not args.retrain else False,
                         slice_size=args.slice_size, clinic_size=args.clinic_size,
                         **kwargs)
...
DATA_ROOT = os.path.expanduser(r'C:\Users\onekey\Project\OnekeyDS\CT')
