...
def train_one_epoch(model, device, train_loader: DataLoader, criterion, optimizer, lr_scheduler, writer, epoch,
    ...
    for idx, batch_data in enumerate(train_loader):
        ...
        probabilities, predictions = torch.max(probs, 1)
        for idx_, (prob, probability, prediction, label) in enumerate(zip(probs, probabilities, predictions, labels)):
            train_file.write('%s\t%.3f\t%d\t%d\n' % (os.path.basename(fnames[fname_idx + idx_]),
                                                     probability, prediction, label))
            train_file_spec.write('%s,%s\n' % (os.path.basename(fnames[fname_idx + idx_]),
                                               ','.join(map(str, prob.detach().cpu().numpy()))))
...
def evaluate(model, device, val_loader, writer, epoch, **kwargs):
    ...
    val_file_spec = open(os.path.join(val_dir, f'Epoch-{epoch}_spec.csv'), 'w')
    ...
    with torch.no_grad():
        ...
        for val_data in val_loader:
            ...
            probs = nn.functional.softmax(val_outputs, dim=1)
            probabilities, predictions = torch.max(probs, 1)
            for idx_, (prob, probability, prediction, label) in enumerate(
                    zip(probs, probabilities, predictions, labels)):
...
def main(args):
    ...
    # Define transforms
    train_transforms = Compose([ScaleIntensity(), AddChannel(), Resize(args.roi_size), EnsureType()])
    ...
    try:
        ...
        in_channels = 1
    model = create_model(f'classification3d.{args.model_name}', in_channels=in_channels,
                         num_classes=train_datasets.num_classes, pretrained=args.pretrained,
                         **args.model_config).to(device)
    ...
    epoch_loss_values = []
    metric_values = []
    model_dir = create_dir_if_not_exists(os.path.join(args.model_root, args.model_name), add_date=False)
