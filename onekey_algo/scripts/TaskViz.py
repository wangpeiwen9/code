...
from onekey_algo.classification.eval_classification import init as init_clf, inference as inference_clf
from onekey_algo.detection.eval_detection import init as init_obj, inference as inference_obj
from onekey_algo.scripts.core import draw_roc, draw_confusion_matrix
from onekey_algo.segmentation.eval_segmentation import init as init_seg, inference as inference_seg
...
TaskTuple = collections.namedtuple('TaskTuple', ['sub_viz_dir', 'model_path', 'onekey_results',
...
def get_log(model_root, task_type=None):
    ...
    assert task_type in ['what', 'where', 'which'], "目前仅支持Onekey中的what、where、which类任务分析！"
    model_path = os.path.join(model_root, 'BEST-training-params.pth')
    onekey_results = os.path.join(model_root, 'BST_VAL_RESULTS.txt')
    ...
    if task_type is None or task_params['type'] == task_type:
        return TaskTuple(os.path.abspath(model_root), model_path, onekey_results, labels, label_path,
    ...
    def viz(self):
        ...
        for k, v in self.results.items():
            ...
            if self.sub_viz[k].task_params['num_classes'] == 2:
                auc = draw_roc(self.sub_viz[k].onekey_results, save_dir=os.path.join(self.save_dir, k))
                viz_spec['result_list'].append({'img': os.path.join(self.save_dir, k, f"roc.svg"),
                                                'desc': f"ROC曲线, auc={auc}"})
                plt.close()
            draw_confusion_matrix(self.sub_viz[k].onekey_results, self.sub_viz[k].label_path,
...
class DetectionVisualize(TaskVisualize):
    ...
    def viz(self):
        ...
        for k, v in self.results.items():
            ...
            df = pd.read_csv(sub_viz.onekey_results, header=0, index_col=0)
...
class SegmentationVisualize(TaskVisualize):
    ...
    def viz(self):
        ...
        for k, v in self.results.items():
            ...
            plt.savefig(os.path.join(self.save_dir, k, f"metric.svg"), dpi=300)
            plt.close()
            # content = open(sub_viz.onekey_results, encoding='utf8').read()
            content = ''
            viz_spec['result_list'].append({'img': os.path.join(self.save_dir, k, f"metric.svg"),
