...
from onekey_algo.utils import split_dataset
from onekey_algo.utils import truncate_dir, create_dir_if_not_exists
...
def get_labels_file(input_dir, recursive: bool = True):
    if isinstance(input_dir, str) and os.path.exists(input_dir):
        if recursive:
            label_files = []
        ...
        label_files = [i for i in input_dir if i.endswith('.json')]
    else:
        raise ValueError(f"{input_dir} 不是文件列表，也不是目录，请检查设置。")
...
def seg_covert2voc(input_dir: str = None, save_dir: str = '.', partition: List[float] = [0.8, 0.2], noviz: bool = False,
    ...
    for subset, subset_name in zip([train, val], ['train', 'val']):
        ...
        for image_id, filename in enumerate(subset):
            ...
            if (len(img.shape) != 3 or img.shape[-1] != 3) and len(img.shape) != 2:
                logger.error(f'\t{filename}数据格式不匹配，解析失败！')
                continue
            data["images"].append(
                dict(
                    license=0,
    ...
    commands = [r'cd C:\Users\onekey\.conda\envs\onekey\Lib\site-packages\onekey_algo/segmentation',
...
if __name__ == "__main__":
    ...
    args.input_dir = r'C:\Users\onekey\Desktop\1'
    args.save_dir = r'C:\Users\onekey\Desktop\2'
