...
class MetricLogger(object):
    def __init__(self, delimiter="\t", log_dir=None, epoch=0):
        self.meters = defaultdict(SmoothedValue)
        self.delimiter = delimiter
        self.training_log = log_dir
        self.epoch = epoch
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0:
                ...
                log_ = [f"Epoch-{self.epoch}-{i}"] + [str(m) for m in self.meters.values()]
                if self.training_log:
                    print(','.join(log_), file=self.training_log)
                if torch.cuda.is_available():
    ...
    for g, (grp, tensors) in enumerate(tensors.items()):
        for k, tensor in enumerate(tensors):
            ...
            if len(np.unique(x)) > 10:
                ...
                colors = np.array(get_color_map_list(int(np.max(x)) + 1)).reshape(-1).tolist()
