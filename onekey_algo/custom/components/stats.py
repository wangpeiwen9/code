...
def clinic_stats_dual(sdata: DataFrame, stats_columns: Union[str, List[str]], label_column='label',
                      group_column: str = None, continuous_columns: Union[str, List[str]] = None,
                      pretty: bool = True, label_spec: bool = True, test_type: str = None,
    ...
    for c in stats_columns:
        ...
        for g, d in data.items():
            ...
            shapiro_results = shapiro(d[c])
            try:
                s_pvalue = shapiro_results.pvalue
            except:
                s_pvalue = shapiro_results[0]
            if s_pvalue > 0.05:
                test_type = test_type or 'ttest'
            else:
                test_type = test_type or 'utest'
            if len(labels) == 2:
                if test_type == 'ttest':
                    if verbose:
                        print(f'Feature {c} using ttest，{shapiro_results}...')
                    p_value = ttest_ind(d[d[label_column] == labels[0]][c], d[d[label_column] == labels[1]][c]).pvalue
                else:
                    if verbose:
                        print(f'Feature {c} using utest，{shapiro_results}...')
