...
def get_image_mask_from_dir(path, images=None, masks=None):
    ...
    if not os.path.exists(os.path.join(path, images)) or not os.path.exists(os.path.join(path, masks)):
        raise ValueError(f"请检查你的配置，在{path}目录，我们没有找到{images}和{masks}文件夹！")
    images_path = Path(os.path.join(path, images))
    masks_path = Path(os.path.join(path, masks))
    images = []
    masks = []
    for l_ in os.listdir(images_path):
        if not l_.startswith('.') and os.path.isfile(os.path.join(images_path, l_)):
            ...
            if len(mask_file) == 1:
                ...
                logger.warning(
                    f"我们找到多个与{l_}有共同pattern的文件, 他们是{[os.path.basename(l) for l in mask_file]}， "
                    f"我们将使用完全匹配获取数据。")
