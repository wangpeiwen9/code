...
from onekey_algo.segmentation.model_zoo.pranet.utils.dataloader import test_dataset
...
model = create_model('segmentation.PraNet')
model.load_state_dict(torch.load(opt.pth_path))
logger.info(f"Model loaded from {opt.pth_path}")
model.cuda()
model.eval()
...
for _data_name in ['CVC-300', 'CVC-ClinicDB', 'Kvasir', 'CVC-ColonDB', 'ETIS-LaribPolypDB']:
    ...
    logger.info(f'\tPredicting {_data_name} and save to {save_path}')
