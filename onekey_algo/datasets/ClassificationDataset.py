...
from onekey_algo.utils.common import check_file_exists, common_annotation_parser, \
    ...
    get_files_in_dir, common_annotation_parser25d
import numpy as np
...
class ListDataset25D(data.Dataset):
    """
    The training or validation dataset is stored in a or a list of  isolate file.
    One must pass `record_parser` to parse record, _common_record_parser is used by default.
    """
    ...
    def __init__(self, records: List[str], record_parser=None, labels_file: str = None, classes: list = None,
                 ori_img_root: str = None, max2use: int = None, batch_balance: bool = False,
                 loader=None, transform=None, target_transform=None, check_sample_exists: bool = True,
                 fname_len: int = 1, retry: int = None, **kwargs):
        """
        Initial function for ListDataset. Do the following thing.
        ...
        :param check_sample_exists: Whether to check image file exists
            default True which may slow down the creation of dataset.
        :param retry: How much time to re get samples if `index` failed. RANDOM index will be used!
        :param kwargs: Unused args.
        """
        self.file_names = []
        self.labels = []
        self.clinic_features = []
        self.cp = ColorPrinter()
        self.fname_len = fname_len
        del kwargs
        if record_parser is None:
            record_parser = partial(common_annotation_parser25d, fname_len=self.fname_len, sep='\t')
        if loader is None:
            loader = default_loader
        if isinstance(records, str):
            records = [records]
        ...
        for record in records:
            logger.info(self.cp.color_text(f'Parsing record file {record}'))
            file_name_, label_, clinic_feature_ = record_parser(record)
            if check_sample_exists:
                logger.info(f"\tChecking file exists in {record}")
            else:
                logger.info(f"\tSkip Checking file exists in {record}")
            for file_name, label, clinic_feature in zip(file_name_, label_, clinic_feature_):
                if check_sample_exists:
                    for fname in file_name:
                        if not check_file_exists(fname, ori_img_root):
                            raise ValueError(f"{fname}不存在！")
                self.file_names.append(file_name)
                self.labels.append(label)
                self.clinic_features.append(clinic_feature)

        # Cut samples and batch balance it if necessary.
        if max2use is not None or batch_balance:
            _labels_samples = {}
            for file_name, label, clinic_feature in zip(self.file_names, self.labels, self.clinic_features):
                if label not in _labels_samples:
                    _labels_samples[label] = []
                _labels_samples[label].append((file_name, clinic_feature))
            ...
            self.clinic_features = []
            for l in _labels_samples:
                for file_name, clinic_feature in _labels_samples[l]:
                    self.file_names.append(file_name)
                    self.labels.append(l)
                    self.clinic_features.append(clinic_feature)
    ...
    def parse_25d_data(self, file_path):
        samples_ = []
        for fp in file_path:
            sample_ = self.loader(fp)
            if self.transform is not None:
                sample_ = self.transform(sample_)
            samples_.append(sample_.unsqueeze(0))

        samples_ = torch.cat(samples_, dim=0)
        return samples_, file_path[0]

    def __get_item(self, index):
        label_ = self.labels[index]
        sample_, path_ = self.parse_25d_data(self.file_names[index])
        clinic_features_ = torch.tensor(self.clinic_features[index], dtype=torch.float32)
        if self.target_transform is not None:
            label_ = self.target_transform(label_)
        return sample_, clinic_features_, label_, path_
    ...
    def __getitem__(self, index: int):
        ...
        :param index: Index
        :return: (sample, clinic_feature, label) where label is class_index of the target class.
        """
        try:
            return self.__get_item(index)
        except Exception as e:
    ...
    if dataset_name is None:
        ...
        if 'is_25d' in kwargs:
            if 'records' in kwargs and kwargs['records'] is not None:
                dataset = ListDataset25D(**kwargs)
            else:
                raise ValueError(f"2.5D不支持Folder模式！")
        else:
            if 'records' in kwargs and kwargs['records'] is not None:
