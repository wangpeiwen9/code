...
class NIH_Dataset(Dataset):
    ...
    def __init__(self, imgpath,
        ...
        self.pathology_maskscsv["Finding Label"][
            self.pathology_maskscsv["Finding Label"] == "Infiltrate"] = "Infiltration"
...
class PC_Dataset(Dataset):
    """
    PadChest dataset
    Hos.pathital San Juan de Alicante - University of Alicante
...
class COVID19_Dataset(Dataset):
    """
    COVID-19 Image Data Collection: Pros.pathective Predictions Are the Future
    ...
    def __init__(self, data_path,
        ...
        self.semantic_masks_v7labs_lungs_path = os.path.join(data_path, 'annotations',
                                                             "semantic_masks_v7labs_lungs.zip")

        # Load data
        self.csvpath = os.path.join(data_path, "metadata.csv")
        self.csv = pd.read_csv(self.csvpath, nrows=nrows)
        ...
        if self.semantic_masks:
            ...
            imgid = [os.path.basename(name) for name in self.semantic_masks_v7labs_lungs_namelist]
            self.csv = self.csv[self.csv['filename'].isin(imgid)]
    ...
    def __repr__(self):
        head = f"Dataset is COVID19 Dataset for task {'segmentation' if self.semantic_masks else 'classification'}."
        body = ["Samples: {}, Classes: {}, views={}".format(self.__len__(), self.num_classes, self.views)]
        if self.data_path is not None:
            body.append("Root location: {}".format(self.data_path))
    ...
    def __getitem__(self, idx):
        ...
        if self.semantic_masks:
            target = self.get_semantic_mask_dict(imgid)
            if self.transform is not None:
    ...
    def get_semantic_mask_dict(self, image_name):
        ...
        if archive_path in self.semantic_masks_v7labs_lungs_namelist:
            with zipfile.ZipFile(self.semantic_masks_v7labs_lungs_path).open(archive_path) as file:
                ...
                semantic_masks = np.where(mask == 255, 1, 0)
            return Image.fromarray(semantic_masks.astype(np.float))
        else:
...
def create_xray_dataset(name, **kwargs):
    datasets = {'covid-19': COVID19_Dataset}
    assert name in datasets
    return datasets[name](**kwargs)


if __name__ == '__main__':
    ds = COVID19_Dataset('/Users/zhangzhiwei/projects/python/covid-chestxray-dataset', semantic_masks=True)
    print(ds)
    # for d in ds:
    #     print(d)
