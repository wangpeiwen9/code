...
class FlatVAE(BaseVAE):
    ...
    def __init__(self, feature_len: int, latent_dim: int, hidden_dims: List = None, ori_comp_dim: int = None,
        ...
        self.hidden_dims = [h for h in hidden_dims]
        # Build Encoder
        in_channels = 1
        self.ori_comp_dim = ori_comp_dim
        if self.ori_comp_dim:
            self.fc_first = nn.Linear(feature_len, ori_comp_dim)
            self.fc_last = nn.Linear(ori_comp_dim, feature_len)
            feature_len = ori_comp_dim
        else:
            self.fc_first = None
            self.fc_last = None
...
if __name__ == '__main__':
    f_len = 2580
    model = FlatVAE(f_len, 32, kernel_size=5, stride=1, ori_comp_dim=1024)
