...
from onekey_core.models.classification3d import resnet50, resnet18
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        def _create_block(
            ...
            if len(channels) > 2:
                subblock, _ = _create_block(c, c, channels[1:], strides[1:], False)  # continue recursion down
                upc = c * 2
            else:
            ...
            return nn.Sequential(down, SkipConnection(subblock), up), down

        self.model, self.down_model = _create_block(in_channels, out_channels, self.channels, self.strides, True)
        print(self.model)
...
class FusionUNet(nn.Module):
    def __init__(
            ...
            **kwargs) -> None:
        super().__init__()
        self.seg_model = UNet(spatial_dims=spatial_dims, in_channels=in_channels, out_channels=out_channels,
                              channels=channels, strides=strides, kernel_size=kernel_size,
                              up_kernel_size=up_kernel_size, num_res_units=num_res_units,
                              act=act, norm=norm, dropout=dropout, bias=bias, **kwargs)
        self.clf_model = resnet18(num_classes=num_classes)
        # self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
        # self.fc = nn.Linear(channels[0], num_classes)

    def forward(self, x: torch.Tensor) -> (torch.Tensor, torch.Tensor):
        # clf = self.avgpool(self.seg_model.down_model(x))
        # clf_task = self.fc(torch.flatten(clf, 1))
        seg_task = self.seg_model(x)
        clf_input = torch.cat([seg_task[:, 1:2, ...], x, seg_task[:, 1:2, ...] * x], dim=1)
        clf_task = self.clf_model(clf_input)
        return seg_task, clf_task


if __name__ == '__main__':
    a = torch.rand((3, 1, 96, 96, 96))
    m = FusionUNet(
        spatial_dims=3,
        in_channels=1,
        out_channels=2,
